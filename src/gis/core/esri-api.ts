...
export async function createMap<T extends __esri.MapProperties>(
  properties: any
): Promise<__esri.Map> {
  await castLayers(properties);
  const [EsriMap] = await loadModules(['esri/Map']);
  // await castLayers(properties);
  return new EsriMap(properties);
}
...
>(view: __esri.View, layer: TLayer): Promise<TLayerView> {
...
export async function buffer(
  ...
  const geoSvc: __esri.GeometryService = new GeometryService({
    url: geometryServiceUrl,
  });
  const params: __esri.BufferParameters = new BufferParameters(properties);
...
export async function parseRendererFromJSON<T extends __esri.Renderer>(
  json: any
): Promise<T> {
  const [jsonUtils] = await loadModules(['esri/renderers/support/jsonUtils']);
...
export async function createSlideFromView(
  ...
  const [Slide] = await loadModules(['esri/webscene/Slide']);
...
export async function createPopupTemplate(
  ...
  const [PopupTemplate] = await loadModules(['esri/PopupTemplate']);
...
export async function createField(
  ...
  const [Field] = await loadModules(['esri/layers/support/Field']);
...
export async function parseLabelClassFromJSON(
  ...
  const [LabelClass] = await loadModules(['esri/layers/support/LabelClass']);
...
export async function parseSymbolFromJson<T extends __esri.Symbol>(
  ...
  const [jsonUtils] = await loadModules(['esri/symbols/support/jsonUtils']);
  return jsonUtils.fromJSON(json);
}
...
export async function parsePresentationFromJSON(
  ...
  const [Presentation] = await loadModules(['esri/webscene/Presentation']);
...
export async function parseFeatureSetFromJson(
  ...
  const [FeatureSet] = await loadModules(['esri/tasks/support/FeatureSet']);
...
export async function createCollection<T>(
  ...
  const [Collection] = await loadModules(['esri/core/Collection']);
...
async function castLayers<T extends __esri.MapProperties>(properties: any) {
  // layers
...
export function createExaggeratedElevationLayer(properties: {
  url: string;
  exaggeration: number;
...
export async function flyTo(
  ...
  target:
    ...
    | __esri.Geometry
    | __esri.Geometry[]
    | __esri.Collection<__esri.Geometry>
    | __esri.Graphic
    | __esri.Graphic[]
